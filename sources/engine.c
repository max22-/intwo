...
int eliminate_cut(System *s)
{
	...
	if ( cut )
	{
		...
		if (ruleList==NULL)  // rule not found
		{
			printf("Rule %s:%d >< %s:%d not found\n",
					LABEL(CUT_PTR(cut)->cell1), ARITY(CUT_PTR(cut)->cell1),
					LABEL(CUT_PTR(cut)->cell2), ARITY(CUT_PTR(cut)->cell2)
				  );
			return 0;
		}
		rule = RULE_PTR(ruleList->data);

		g_print("\nApply rule %s >< %s...", rule->cell1->symbol->label, rule->cell2->symbol->label);
